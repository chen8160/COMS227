...
public class HangmanUI extends JPanel {
	...
	/**
	 * State variable indicating whether the figure should be drawn with a bow
	 * or a hat.
	 */
	private boolean drawHat;

	/**
	 * State variable indicating whether the figure should be drawn wearing a
	 * skirt.
	 */
	private boolean drawSkirt;

	/**
	 * Main entry point for starting up an instance of this user interface.
	 * 
	 * @param selector
	 ...
	 *             if the file referred to by the PhraseSelector cannot be
	 *             opened
	 */
	private HangmanUI(PhraseSelector selector, Random rand)
			throws FileNotFoundException {
	...
	private class PlayAgainHandler implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			try {
				...
				JOptionPane.showMessageDialog(HangmanUI.this.getParent(),
						"Unable to open file: " + fnfe.getMessage());
			}
		}
	}
   ...
   */
	...
	private class WordCanvas extends JPanel {
		...
		public void paintComponent(Graphics g) {
			...
			for (int i = 0; i < chars.length; ++i) {
				letters[i] = new LetterRectangle(LETTER_WIDTH, LETTER_HEIGHT,
						chars[i].getDisplayedChar());
			}
			...
			int windowWidth = Math.max(getWidth(), totalWidth + 2
					* LETTER_WIDTH);
			...
			for (int i = 0; i < letters.length; ++i) {
				...
				if (text != null) {
					...
					// g2.fillRect(r.x + 1, r.y + 1, LETTER_WIDTH - 2,
					// LETTER_HEIGHT - 2);
	...
	private class GameCanvas extends JPanel {
		...
		public void paintComponent(Graphics g) {
			...
			//
			// Draw the stick figure from the top down based on the number of
			// wrong guesses
			//
			...
			if (wrongGuesses >= 3) {
				...
				g2.drawLine(center, currentTop, center - armWidth, currentTop
						+ armHeight);
			}
			...
			if (wrongGuesses >= 6) {
				...
				g2.drawLine(center, currentTop, center - legWidth, currentTop
						+ legHeight);
			}
		...
		private void drawHead(Graphics2D g2, boolean dead) {
			...
			if (!dead) {
				// regular eyes
				g2.drawOval(center - eyeWidth - eyeRadius, headTop + eyeHeight
				...
				g2.drawLine(center - eyeWidth - eyeRadius, headTop + eyeHeight,
						center - eyeWidth + eyeRadius, headTop + eyeHeight);
				g2.drawLine(center - eyeWidth, headTop + eyeHeight - eyeRadius,
						center - eyeWidth, headTop + eyeHeight + eyeRadius);
				g2.drawLine(center + eyeWidth - eyeRadius, headTop + eyeHeight,
						center + eyeWidth + eyeRadius, headTop + eyeHeight);
				g2.drawLine(center + eyeWidth, headTop + eyeHeight - eyeRadius,
						center + eyeWidth, headTop + eyeHeight + eyeRadius);
			...
			// mouth
			g2.drawLine(center - mouthWidth, headTop + mouthHeight, center
					+ mouthWidth, headTop + mouthHeight);
		}
		...
		private void drawHat(Graphics2D g2) {
			...
			// top of hat is a rounded rectangle filled out at bottom with a
			// polygon
			...
			// brim
			g2.drawLine(center - 70, headTop + height / 2 - 5, center + 60,
					headTop + height / 2 - 12);
		}
		...
		private void drawSkirt(Graphics2D g2) {
			...
			int dressHeight = 120;
			int[] xPoints = { center - dressWidthTop,
					center - dressWidthBottom, center + dressWidthBottom,
					center + dressWidthTop };
		...
		private void drawScaffold(Graphics2D g2) {
			...
			g2.drawLine(scaffoldX + scaffoldThickness, scaffoldY
					+ scaffoldWidth / 3, scaffoldX + scaffoldWidth / 3,
					scaffoldY + scaffoldThickness);
